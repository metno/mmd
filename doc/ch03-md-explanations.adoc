...
The MMD specification uses a complex field platfrom (see <<platform>>) to store this.
...
In addition, vocabularies are not always complete, have might have gaps, that cannot be covered using a single source.

Depending on the type of data, some information might not be available. Below an overview of the supported vocabularies, and recommendations
on how to fill the platform/instrument fields are provided.

Remote Sensing::

For remote sensing data, platform and instrument the WMO https://space.oscar.wmo.int/spacecapabilities[Space-based Capabilities (OSCAR/Space)] reference is recommended.
