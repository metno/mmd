  ...
  -->
    ...
    <xsl:template match="dif:Data_Center">
        ...
        <xsl:element name="mmd:personnel">
            ...
            <xsl:choose>
                <xsl:when test="normalize-space(dif:Data_Center_Name/dif:Long_Name) !=''">
                    <xsl:call-template name="organisation">
                        <xsl:with-param name="myorg" select="normalize-space(dif:Data_Center_Name/dif:Long_Name)" />
                    </xsl:call-template>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:call-template name="organisation">
                        <xsl:with-param name="myorg" select="normalize-space(dif:Data_Center_Name/dif:Short_Name)" />
                    </xsl:call-template>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:element>
    ...
    <xsl:template match="dif:Organization">
        <xsl:if test="dif:Organization_Type[contains(text(),'ARCHIVER') or contains(text(),'DISTRIBUTOR') or contains(text(),'ORIGINATOR')]">
            ...
            <xsl:element name="mmd:personnel">
                ...
                <xsl:choose>
                    <xsl:when test="dif:Personnel/dif:Contact_Person">
                        <xsl:choose>
                            <xsl:when test="dif:Organization_Name/dif:Long_Name !=''">
                                <xsl:call-template name="organisation">
                                    <xsl:with-param name="myorg" select="dif:Organization_Name/dif:Long_Name" />
                                </xsl:call-template>
                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:call-template name="organisation">
                                    <xsl:with-param name="myorg" select="dif:Organization_Name/dif:Short_Name" />
                                </xsl:call-template>
                            </xsl:otherwise>
                        </xsl:choose>
                        <!--xsl:value-of select="dif:Organization_Name/dif:Long_Name"/-->
                    </xsl:when>
                    <xsl:when test="dif:Personnel/dif:Contact_Group">
                        <xsl:call-template name="organisation">
                            <xsl:with-param name="myorg" select="dif:Personnel/dif:Contact_Group/dif:Name" />
                        <!--xsl:value-of select="dif:Personnel/dif:Contact_Group/dif:Name"/-->
                        </xsl:call-template>
                    </xsl:when>
                    <xsl:otherwise>
    ...
    <xsl:template match="dif:Personnel">
        <xsl:element name="mmd:personnel">
            ...
            <xsl:if test="dif:Contact_Person">
                <xsl:element name="mmd:type">
                    <xsl:text>Person</xsl:text>
                </xsl:element>
            </xsl:if>
            <xsl:if test="dif:Contact_Group">
                <xsl:element name="mmd:type">
                    <xsl:text>Organisation</xsl:text>
                </xsl:element>
            </xsl:if>
...
-->
    ...
    <!--Data center short/long-->
    <xsl:template name="datacenter">
    <xsl:param name="mydcs"/> <!--always filled, but can be long name-->
    <xsl:param name="mydcl"/>
        <xsl:for-each select="$vocab" >
            <xsl:choose>
                <xsl:when test="key('orgeng', $mydcl)">
                    <xsl:choose>
                        <xsl:when test="$orguri != ''">
                            <xsl:element name="mmd:organisation">
                                <xsl:variable name="orgpref" select="key('orgeng', $myorg)/skos:prefLabel"/>
