...
<!--Translation from MMD to DIF 10.2-->
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/mmd:mmd">
        <xsl:element name="dif:DIF">
            ...
            <!-- Project/Short_Name is mandatory-->
            <xsl:element name="dif:Project">
                <xsl:choose>
                    <xsl:when test="mmd:project">
                        <xsl:element name="dif:Short_Name">
                            <xsl:value-of select="mmd:project/mmd:short_name" />
                        </xsl:element>
                        <xsl:if test="mmd:project/mmd:long_name">
                            <xsl:element name="dif:Long_Name">
                                <xsl:value-of select="mmd:project/mmd:long_name" />
                            </xsl:element>
                        </xsl:if>
                    </xsl:when>
                    <xsl:otherwise>
            ...
            <xsl:element name="dif:Metadata_Version">VERSION 10.2</xsl:element>
            <!--Medata_Dates/Data* are mandatory-->
            <xsl:element name="dif:Metadata_Dates">
                <xsl:apply-templates select="mmd:last_metadata_update" />
                <xsl:element name="dif:Data_Creation">
                    <xsl:text>unknown</xsl:text>
                </xsl:element>
                <xsl:element name="dif:Data_Last_Revision">
                    <xsl:text>unknown</xsl:text>
                </xsl:element>
            </xsl:element>
        </xsl:element>
            ...
            <xsl:element name="dif:Personnel">
                ...
                <xsl:element name="dif:Contact_Group">
                    ...
                    <xsl:element name="dif:Email">
                        <xsl:value-of select="../mmd:personnel/mmd:email[../mmd:role='Data center contact']" />
                    </xsl:element>
                </xsl:element>
            </xsl:element>
        </xsl:element>
    ...
    <xsl:template match="mmd:personnel">
        <xsl:element name="dif:Personnel">
            ...
            <xsl:element name="dif:Contact_Person">
